...
from pidal.dservice.transaction.a2pc.client.constant import A2PCOperation,\
        A2PCStatus
from pidal.dservice.transaction.a2pc.client.client import A2PCResponse,\
        A2PClient
...
class A2PC(Trans):
    ...
    async def execute_select(self, sql: Select) -> result.Result:
        ...
        if isinstance(r, result.ResultSet):
            self._record_undo_data(table, lock_keys, r)
        return r

    def _record_undo_data(self, table: Table, lock_keys: Dict[str, Any],
                          old_raw: Optional[result.ResultSet]):
        old = old_raw.to_dict()
        if not old or len(old) > 1:
            raise Exception("row need unique.")
        old = old[0]

        for table_name in table.get_real_table(old):
            ...
            if not log:
                log = ReUnFactory.new("unique", self.xid, lock_keys,
                                      table_name)
                print("===")
                print(str(lock_keys), table_name)
                self._reundo_log[table_name][str(lock_keys)] = log
            log.set_undo(old)

    def _record_undo_data_raw(self, table: str, lock_keys: Dict[str, Any],
                              old_raw: Optional[result.ResultSet]):
        if old_raw:
            old = old_raw.to_dict()
            if not old or len(old) > 1:
    ...
    async def execute_update(self, sql: Update) -> result.Result:
        ...
        c = []
        for i in node:
            self._modify_table(table, i, sql)
            error = await self._update_undo_log(table, lock_keys, sql)
            if error:
            ...
            c.append(self._execute_dml(i, before_sql, str(sql.raw)))
    ...
    async def _execute_dml(self, node: DBTableStrategyBackend,
                           before_sql: str, sql: str) -> result.Result:
        backend = await self.backend_manager.get_backend(node.node, self.xid)
        return await backend.query(before_sql + sql)

    @staticmethod
    def _modify_table(table: Table, node: DBTableStrategyBackend, sql: DML):
        sql.modify_table(node.prefix + str(node.number))
        sql.add_pidal(table.get_pidal_c_v())

    async def _update_undo_log(self, table: Table, lock_keys: Dict[str, Any],
        ...
        if log is None:
            ...
            self._record_undo_data(table, lock_keys, old_raw)
    ...
    async def _insert_undo_log(self, lock_keys: Dict[str, Any],
        ...
        if log is None:
            self._record_undo_data_raw(table_name, lock_keys, None)
            log = self._reundo_log[table_name][str(lock_keys)]
    ...
    async def execute_delete(self, sql: Delete) -> result.Result:
        ...
        for i in node:
            self._modify_table(table, i, sql)
            error = await self._delete_undo_log(table, lock_keys, sql)
            if error:
                return error  # type: ignore
            reundo_sql = self.reundo_log_generator(str(sql.table), lock_keys)
            if not reundo_sql:
    ...
    async def _delete_undo_log(self, table: Table, lock_keys: Dict[str, Any],
    ...
    async def close(self):
        if self.status is not TransStatus.END:
            await self.rollback(None)  # type: ignore
